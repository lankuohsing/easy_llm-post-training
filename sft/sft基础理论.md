...
其中 $\theta$ 表示模型参数， $y_{i,\lt t}$ 表示目标回答中第 $t$ 个 token 之前的所有 token。
...
训练第 $t$ 个 token 时，模型看到的是参考答案的真实前缀 $y_{\lt t}$，而不是自己刚生成的前缀，这称为 **teacher forcing**。它能并行、稳定地训练，却也带来训练与生成之间的差异：推理时模型一旦生成错误，后续 token 将建立在错误前缀上。高质量、多样化的数据和生成式评测因此十分重要，不能只看训练 loss。
